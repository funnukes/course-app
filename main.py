...
try:
    ...
    st.error(f"Error loading CSV file: {e}. This often means the file format or delimiter is incorrect. Please check your CSV file's content and ensure it's semicolon-separated, with 6 lines to skip before the header.")
    st.stop()
...
for code in selected_codes:
    course_row = df[df['Code'] == code] # Find the row for the selected course
    if not course_row.empty:
        # Add all incompatibility codes from this selected course to the set
...
for idx, row in df.iterrows():
    ...
    # Logic to disable courses:
    # 1. If the course is in the incompatible_all set AND it's not already selected
    if code in incompatible_all and not is_selected:
        is_disabled = True
        reason = "❌ Incompatible with selected courses"
    # 2. If the maximum selection limit is reached AND the course is not already selected
    if len(selected_codes) >= 5 and not is_selected:
        is_disabled = True
        reason = "⚠️ Limit reached"

    # Create two columns for each course entry: Course Name and Selectbox
    col1, col2 = st.columns([4, 1])
    with col1:
        st.markdown(f"**{name}** (Code: `{code}`)")
    with col2:
        option = st.selectbox(
            label="", # Label is empty as the course name is displayed in col1
            options=["No", "Yes"],
            index=1 if is_selected else 0, # Set the default selection based on session state
            key=f"decision_{code}", # Unique key for each selectbox widget
            disabled=is_disabled, # Disable the selectbox if conditions are met
            help=reason # Tooltip message when disabled
        )
        st.session_state.selections[code] = option # Update the session state with the user's selection

# --- Final Course Selection Summary ---
final_selected_codes = [c for c, v in st.session_state.selections.items() if v == 'Yes']
# Get the names of the finally selected courses
...
if final_selected_codes:
    ...
    for code in final_selected_codes:
        ...
        incompatible_codes_for_this_selection = row['Incompatible_List']
        for inc_code in incompatible_codes_for_this_selection:
            # Only list incompatibilities that are NOT among the currently selected courses
            if inc_code not in final_selected_codes:
                # Find the course name for the incompatible code
                course_name_series = df[df['Code'] == inc_code]['Course']
...
2. **Incompatible courses** will be automatically disabled (greyed out) when you make selections.
